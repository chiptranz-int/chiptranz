...
class PaymentsController extends Controller
{
    ...
    private $planNotifier;
    public function __construct()
    {
        $this->middleware('auth');
        $this->planNotifier = new PlanNotifierController();
    }
    ...
    public function handleGatewayCall(Request $request, $trxref)
    {
    ...
    public function handleGatewaySubsequentCall(Request $request, $trxref)
    {
            ...
            if (!empty($paymentDetails)) {
                ...
                if ($status == "success") {
                    ...
                    if (!empty($transacts->id)) {
                        ...
                        if (!empty($goal[0])) {
                            $this->saveAmount($goal[0]['id'], $transacts->id, new YouthSaving(), $trxref);
                        } else {
                            $steady = $help->getSteadyGrowth($user->id);
                            if (!empty($steady[0])) {
                                $this->saveAmount($steady[0]['id'], $transacts->id, new SteadySaving(), $trxref);
                            }
                        }
                    }
                }
    ...
    public function saveAmount($id, $transactId, $savings, $ref_no)
    {
        ...
        $savings->status = 1;
        $savings->save();
        $this->planNotifier->newSavingsMade($id, 100, null, $transactId, $ref_no);
    }
    ...
    public function saveUserAmount(Request $request)
    {
        ...
        if (!empty($authCode)) {
            ...
            if ($saved) {
                $this->planNotifier->newSavingsMade($planId, $amount, $planType);

                return response()->json(['success' => true], 200);
            }
        }
        $this->planNotifier->newSavingsMade($planId, $amount, $planType, null, null, 'failure');
